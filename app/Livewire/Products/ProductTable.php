...
final class ProductTable extends PowerGridComponent
{
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ...
            ->add('name_formatted', function (Product $model) {
                return $model->is_active ? $model->name : '(DESCONTINUADO) ' . $model->name;
            })
            ->add('description')
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Categoría', 'category_name', 'category_id')
                ->sortable()
                ->searchable()
                ->visibleInExport(false),

            Column::make('Categoría', 'category_slug', 'category_id')
                ->hidden()
                ->visibleInExport(true),

            Column::make('Unidad', 'unit_symbol', 'unit_id')
                ->sortable()
                ->searchable(),

            Column::make('Precio compra', 'purchase_price_formatted', 'purchase_price')
                ->sortable()
                ->bodyAttribute('text-right'),

            Column::make('Precio venta', 'selling_price_formatted', 'selling_price')
                ->sortable()
                ->bodyAttribute('text-right'),
            ...
            Column::make('Cant.', 'quantity')
                ->sortable()
                ->bodyAttribute('text-center'),

            Column::make('Stock mín.', 'min_stock')
                ->sortable()
                ->bodyAttribute('text-center'),
        ...
        return [
            ...
            Button::add('delete')
                ...
                ->dispatch('open-delete-modal', [
                    ...
                    'description' => "¿Seguro que deseas eliminar el producto '{$row->name}'? Esta acción no se puede deshacer.",
                ])
    ...
    public function delete($rowId, ProductService $service): void
    {
        ...
        if ($product) {
            try {
                ...
                $this->dispatch('toast', message: 'Producto eliminado correctamente.', type: 'success');
            } catch (\Exception $e) {

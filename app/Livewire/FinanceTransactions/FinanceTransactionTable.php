...
final class FinanceTransactionTable extends PowerGridComponent
{
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ...
            ->add('reference_display', function (FinanceTransaction $model) {
                $tag = $model->reference_type ? Tone::badgeHtml(Tone::INFO, 'Auto') : Tone::badgeHtml(Tone::NEUTRAL, 'Manual');
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Categoria', 'category_name', 'finance_category_id')
                ->sortable()
                ->searchable(),
        ...
        if (is_null($row->reference_type)) {
            ...
            $actions[] = Button::add('delete')
                ...
                ->dispatch('open-delete-modal', [
                    ...
                    'description' => "Seguro que deseas eliminar esta transaccion? Esta accion no se puede deshacer.",
                ])
                ->tooltip('Eliminar transaccion');
        }
    ...
    public function bulkPrint(): void
    {
        ...
        // Generar un ID unico para esta sesion de impresion
        $printId = (string) Str::uuid();

        // Almacenar los IDs seleccionados en cache por 5 minutos
        \Illuminate\Support\Facades\Cache::put("finance_print_{$printId}", $checkboxValues, now()->addMinutes(5));

        // Obtener informacion del filtro para pasar
    ...
    public function delete($rowId, FinanceTransactionService $service): void
    {
        ...
        if ($transaction) {
            try {
                ...
                $this->dispatch('toast', message: 'Transaccion eliminada correctamente.', type: 'success');
            } catch (\Exception $e) {
                $message = $e instanceof FinanceTransactionException
                    ? $e->getMessage()
                    : 'Error al eliminar la transaccion: ' . $e->getMessage();

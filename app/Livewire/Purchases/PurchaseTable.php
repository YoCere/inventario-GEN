...
final class PurchaseTable extends PowerGridComponent
{
    ...
    public function setUp(): array
    {
        ...
        return [
            PowerGrid::exportable('compras_exportacion_' . now()->format('Y_m_d'))
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ...
            ->add('date_period', fn() => '') // Campo virtual para filtro
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Número de Factura', 'invoice_number')
                ->searchable()
                ->sortable(),

            Column::make('Proveedor', 'supplier_name', 'supplier_id')
                ->searchable()
                ->sortable(),

            Column::make('Fecha de Compra', 'purchase_date_formatted', 'purchase_date')
                ->sortable(),
    ...
    public function actions(Purchase $row): array
    {
        ...
        // Acción Editar (Solo Borrador o Pedido)
        ...
        // Acción Eliminar (Solo Borrador o Cancelado)
        if (in_array($row->status, [PurchaseStatus::DRAFT, PurchaseStatus::CANCELLED])) {
            $actions[] = Button::add('delete')
                ...
                ->dispatch('open-delete-modal', [
                    ...
                    'description' => "¿Está seguro de que desea eliminar la factura '{$row->invoice_number}'? Esta acción no se puede deshacer.",
                ])
    ...
    public function delete($rowId, PurchaseService $purchaseService): void
    {
        ...
        if ($purchase) {
            try {
                ...
                $this->dispatch('toast', message: 'Compra eliminada correctamente.', type: 'success');
            } catch (PurchaseException $e) {
                $this->dispatch('toast', message: $e->getMessage(), type: 'error');
            } catch (\Exception $e) {
                $this->dispatch('toast', message: 'Ocurrió un error inesperado durante la eliminación.', type: 'error');
            }
        }
    }
}
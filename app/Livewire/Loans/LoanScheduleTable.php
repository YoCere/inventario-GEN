    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ...
            ->add('status_badge', function (LoanInstallment $model) {
                return Tone::badgeHtml($model->status->tone(), $model->status->label());
            });
    }
    ...
    public function actions(LoanInstallment $row): array
    {
        ...
        if ($row->status->value === InstallmentStatus::Pending->value) {
            $actions[] = Button::add('pay')
                ...
                ->class('border border-emerald-200 text-emerald-700 hover:bg-emerald-50 dark:border-emerald-900 dark:text-emerald-400 dark:hover:bg-emerald-950/40 p-2 rounded-md flex items-center justify-center')
